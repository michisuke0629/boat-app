import { getStartTimingByCourse } from '@/lib/aggregate';
import CourseAverageTable from '@/components/CourseAverageTable';

export default async function StartTimingPage({
  params,
}: {
  params: Promise<{ stadiumNumber: string }>;
}) {
  const { stadiumNumber } = await params;
  const { rows, seriesUsed, seriesRequested } = await getStartTimingByCourse(Number(stadiumNumber), 10);
